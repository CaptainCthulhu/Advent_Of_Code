...
for i in guardSleep:
    if sum(guardSleep[i]) > sleepTime:
        sleepTime = sum(guardSleep[i])
        winningGuard = i

maxValue = max(guardSleep[winningGuard])

print(int(winningGuard) * guardSleep[winningGuard].index(maxValue))
